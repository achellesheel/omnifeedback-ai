...
| **HuggingFace Inference Endpoints (paid tier)** | Moves BERT NER / summarization off the free CPU-only Streamlit Cloud tier, cutting the ~95s cold-start (`PRODUCT_REVIEW.md` #8) — **not speculative**: the free tier hit both its memory ceiling ("gone over its resource limits") and, separately, a CPU throttle on 2026-09-13 (PROGRESS.md Milestone 14) from running 4 transformer models with no GPU | ~$0.05–$0.10/hr for a small GPU endpoint, pay-as-you-go | Swap `src/transformer_nlp.py`'s local pipeline calls for HTTP calls to a hosted endpoint |
