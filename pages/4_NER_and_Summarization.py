...
from src.genai_copilot import generate_executive_summary
from src.transformer_nlp import extract_entities

st.set_page_config(page_title="NER & Summarization", page_icon="📝", layout="wide")
st.title("📝 Executive Briefing & Entity Extraction")
st.caption(
    "Uses Claude for genuine thematic synthesis if ANTHROPIC_API_KEY is configured in secrets; otherwise "
    "falls back to DistilBART (`sshleifer/distilbart-cnn-12-6`), which is more extractive on short, "
    "keyword-dense complaint text — see the note below the summary. BERT NER (`dslim/bert-base-NER`) "
    "extracts named entities. Models load lazily on first use — the first click will be slow."
)
...
if st.button("Generate Executive Summary", type="primary"):
    with st.spinner(f"Synthesizing {len(critical_texts)} distinct critical feedback records "
                     f"(first load can take ~30s)..."):
        result = generate_executive_summary(critical_texts)
    st.success("Executive Summary")
    st.write(result["summary"])
    if result["backend"] == "claude":
        st.caption("🔮 Synthesized by Claude — themes and business risk identified across tickets, not sentence extraction.")
    else:
        st.caption(
            "⚙️ Generated by the local DistilBART pipeline (no ANTHROPIC_API_KEY configured). This model "
            "family is trained for span-compression on long-form news articles — on short, keyword-dense "
            "tickets like these, it tends to reuse source sentences rather than synthesize themes across "
            "them. Add ANTHROPIC_API_KEY to Streamlit secrets for genuinely synthesized executive briefs."
        )
